...
const TaskPlanner = () => {
    ...
    const handleGenerate = async (e) => {
        ...
        setPlan(null);
        try {
            const response = await fetch(`${config_1.API_URL}/api/generate-plan`, {
                method: 'POST',
    ...
    return (<div className="container" style={{ marginBottom: '30px', borderLeft: '5px solid #673ab7' }}>
            ...
            <form onSubmit={handleGenerate} style={{ marginTop: '20px' }}>
                <div style={{ marginBottom: '15px' }}>
                    ...
                    <textarea value={goal} onChange={(e) => setGoal(e.target.value)} placeholder="e.g. Build a Vibe Coding iOS App MVP..." required rows="3"/>
                </div>

                <div style={{ marginBottom: '15px' }}>
                    <label style={{ display: 'block', marginBottom: '5px', fontWeight: 'bold', color: '#d1c4e9' }}>Available Roles</label>
                    <input type="text" value={roles} onChange={(e) => setRoles(e.target.value)} placeholder="e.g. Frontend Dev, Designer, QA"/>
                </div>

                <button type="submit" disabled={loading} className="action-btn" style={{ background: loading ? 'rgba(255,255,255,0.1)' : 'linear-gradient(135deg, #673ab7 0%, #512da8 100%)' }}>
            ...
            {/* RESULTS TABLE */}
            {plan && (<div style={{ marginTop: '30px', animation: 'fadeIn 0.5s' }}>
                    ...
                    <table style={{ width: '100%', borderCollapse: 'collapse', marginTop: '10px', color: '#e0e0e0' }}>
                        ...
                        <tbody>
                            {plan.tasks.map((task, index) => (<tr key={index} style={{ borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
